...
def kinloop(timeFrom,timeTo,data,input_rotation_filename):
    ...
    for index, point in enumerate(kinematicsArray[:]): 
        ...
        if len(point) == 5:
            ...
            stageTrench = pygplates.get_equivalent_stage_rotation(fromTimeRotation,toTimeRotation,TRENCHid)
            ...
            #Absolut velocity is the motion of the SP
            stageSP = pygplates.get_equivalent_stage_rotation(fromTimeRotation,toTimeRotation,SPid)
            ...
            #else:
            convRotation = pygplates.get_relative_stage_rotation(fromTimeRotation,toTimeRotation,SPid,TRENCHid)
            poletCONV,angleCONV = convRotation.get_euler_pole_and_angle()
            poleCONV = [poletCONV.get_x(), poletCONV.get_y(), poletCONV.get_z()]

            #The OP motion
            stageOP = pygplates.get_equivalent_stage_rotation(fromTimeRotation,toTimeRotation,OPid)
